...
function Header() {
    ...
    return (
        <header className="border-b-2 h-16 p-5 md:px-10 flex items-center justify-between z-50 ">
            ...
            <nav className="hidden md:flex justify-center items-center">
                    ...
                    buttons.map((btn, i) => {
                        return <section key={i}>
                            <button onClick={() => { navigate(btn.redirect); setSelected(i) }} className={`text-gray-500 w-28 font-semibold px-3 ${i === buttons.length - 1 ? "mr-4" : ""}
                             transition
                            `}>{btn.name}</button>
                            <div className={`border-b-2 relative top-[20px] ${i === selected ?
...

                {
                    ...
                    // using selected 10 so its higher number than amount of buttons in order for transition to go to right side
                        ...
                        :
                        <>
                            <button onClick={() => navigate("/signup")} className="mr-2 rounded-2xl bg-blue-700 text-white font-semibold px-4 py-1">Start for free</button>
                            <button onClick={() => navigate("/signin")} className="rounded-2xl border border-blue-700 text-blue-700 font-semibold px-4 py-1">Sign in</button>
                        </>
                }
            </nav >
        </header >
